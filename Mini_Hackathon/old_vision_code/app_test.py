...
def handle_message(event):
    ...
    if last_message_time is not None:
        ...
        if time_difference.total_seconds() > 10:
            # 执行特定操作（例如发送特定消息）
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text='想叫出選單嗎！'))
            #line_bot_api.reply_message(event.reply_token, TextSendMessage(text='熱烈欢迎！'))
        line_bot_api.reply_message(event.reply_token, TextSendMessage(text='不回来也沒關係！'))
    line_bot_api.reply_message(event.reply_token, TextSendMessage(text='不歡迎你回来！'))
    # 更新上次用户发言时间戳为当前时间
